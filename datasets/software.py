...
class Software(Dataset):
    ...
    def __init__(self, path, fields, **kwargs):
        ...
        super(Software, self).__init__(examples, fields, **kwargs)

    @classmethod
    def splits(cls,
               path,
               train='Software_train.txt',
               validation='Software_dev.txt',
               test='Software_test.txt',
               **kwargs):
        return super(Software, cls).splits(
    ...
    def iters(cls, path, batch_size=64, shuffle=True, device=0, vectors_path=None):
        ...
        if isinstance(cls.TEXT_FIELD, NestedField):
            cls.TEXT_FIELD.nesting_field.vocab = Vectors(name='software_text_200', cache=vectors_path)
        else:
            cls.TEXT_FIELD.vocab = Vectors(name='software_text', cache=vectors_path)
        cls.USR_FIELD.vocab = Vectors(name='software_usr', cache=vectors_path)
        cls.PRD_FIELD.vocab = Vectors(name='software_prd', cache=vectors_path)

        return BucketIterator.splits((train, val, test), batch_size=batch_size, repeat=False, shuffle=shuffle,
                                     sort_within_batch=True, device=device)


class SoftwareHierarchical(Software):
