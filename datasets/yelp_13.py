...
class YELP13(Dataset):
    ...
    def __init__(self, path, fields, **kwargs):
        ...
        super(YELP13, self).__init__(examples, fields, **kwargs)

    @classmethod
    def splits(cls,
               path,
               train='yelp-2013-seg-20-20.train.ss',
               validation='yelp-2013-seg-20-20.dev.ss',
               test='yelp-2013-seg-20-20.test.ss',
               **kwargs):
        return super(YELP13, cls).splits(
    ...
    def iters(cls, path, batch_size=64, shuffle=True, device=0, vectors_path=None):
        ...
        if isinstance(cls.TEXT_FIELD, NestedField):
            cls.TEXT_FIELD.nesting_field.vocab = Vectors(name='yelp_13_text', cache=vectors_path)
        else:
            cls.TEXT_FIELD.vocab = Vectors(name='yelp_13_text', cache=vectors_path)
        cls.USR_FIELD.vocab = Vectors(name='yelp_13_usr', cache=vectors_path)
        cls.PRD_FIELD.vocab = Vectors(name='yelp_13_prd', cache=vectors_path)

        return BucketIterator.splits((train, val, test), batch_size=batch_size, repeat=False, shuffle=shuffle,
                                     sort_within_batch=True, device=device)


class YELP13Hierarchical(YELP13):
