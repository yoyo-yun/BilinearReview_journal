...
class Industrial(Dataset):
    ...
    def __init__(self, path, fields, **kwargs):
        ...
        super(Industrial, self).__init__(examples, fields, **kwargs)

    @classmethod
    def splits(cls,
               path,
               train='Industrial_and_Scientific_train.txt',
               validation='Industrial_and_Scientific_dev.txt',
               test='Industrial_and_Scientific_test.txt',
               **kwargs):
        return super(Industrial, cls).splits(
    ...
    def iters(cls, path, batch_size=64, shuffle=True, device=0, vectors_path=None):
        ...
        if isinstance(cls.TEXT_FIELD, NestedField):
            cls.TEXT_FIELD.nesting_field.vocab = Vectors(name='industrial_text_200', cache=vectors_path)
        else:
            cls.TEXT_FIELD.vocab = Vectors(name='industrial_text', cache=vectors_path)
        cls.USR_FIELD.vocab = Vectors(name='industrial_usr', cache=vectors_path)
        cls.PRD_FIELD.vocab = Vectors(name='industrial_prd', cache=vectors_path)

        return BucketIterator.splits((train, val, test), batch_size=batch_size, repeat=False, shuffle=shuffle,
                                     sort_within_batch=True, device=device)


class IndustrialHierarchical(Industrial):
